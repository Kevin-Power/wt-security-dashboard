...
export function RiskGauge({ score, level, color, size = 'lg' }: RiskGaugeProps) {
  const sizes = {
  ...
  // Use provided color or determine based on level
  const strokeColor =
    color ||
    (level === 'critical'
      ? '#ef4444'
    ...
    <div className="flex flex-col items-center">
      <div className="relative" style={{ width, height: height / 2 + 20 }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-2">
          <span className={cn('font-bold', fontSize)} style={{ color: strokeColor }}>
            {score}
          </span>
